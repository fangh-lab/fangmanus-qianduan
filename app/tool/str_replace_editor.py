...
class StrReplaceEditor(BaseTool):
    ...
    parameters: dict = {
        ...
        "properties": {
            ...
            "view_range": {
                "description": "Optional parameter of `view` command when `path` points to a file. If none is given, the full file is shown. If provided, the file will be shown in the indicated line number range, e.g. [11, 12] will show lines 11 and 12.",
                "items": {"type": "integer"},
                "type": "array",
            },
        },
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        # 处理路径：如果是相对路径，转换为基于 workspace 的绝对路径
        input_path = Path(path)
        if not input_path.is_absolute():
            resolved_path = Path(config.workspace_root) / input_path
        else:
        ...
        if command == "view":
            result = await self.view(str(resolved_path), view_range, operator)
        elif command == "create":
            if file_text is None:
                raise ToolError("Parameter `file_text` is required for command: create")
            await operator.write_file(str(resolved_path), file_text)
            ...
            result = await self.str_replace(str(resolved_path), old_str, new_str, operator)
        elif command == "insert":
            ...
            result = await self.insert(str(resolved_path), insert_line, new_str, operator)
        elif command == "undo_edit":
            result = await self.undo_edit(str(resolved_path), operator)
        else:
    ...
    async def _view_directory(path: PathLike, operator: FileOperator) -> CLIResult:
        ...
        from pathlib import Path
        import sys
        def list_files_recursive(current_path, max_depth, current_depth=0):
            ...
            try:
                for item in sorted(Path(current_path).iterdir()):
                    ...
                    items.append(str(item.relative_to(current_path)))

                    if item.is_dir() and current_depth < max_depth:
                        sub_items = list_files_recursive(item, max_depth, current_depth + 1)
                        for sub_item in sub_items:
                            items.append(f"{item.name}/{sub_item}")
            except PermissionError:
                pass

            return items

        files = list_files_recursive(path, max_depth=2)
    ...
    async def _view_file(
        ...
        if view_range:
            ...
            # Apply range
            if final_line == -1:
                file_content = "\n".join(file_content[init_line - 1 :])
            else:
                file_content = "\n".join(file_content[init_line - 1 : final_line])
